...
// cliente HTTP customizado para o Orval
export class createOrvalHttpClient implements IHttpClient {
...
// Função mutator esperada pelo Orval.
export const orvalHttpClient = async <TResponse, TBody = unknown>(
  config: AxiosRequestConfig,
): Promise<TResponse> => {
  // utiliza o cliente customizado do axios
  const client = new createOrvalHttpClient();

  // valida os métodos HTTP
  const method = (config.method ?? 'get')
    .toString()
    .toLowerCase() as HttpMethod;

  // converte AxiosRequestConfig -> HttpRequest<TBody>
